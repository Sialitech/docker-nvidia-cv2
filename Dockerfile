FROM docker.io/nvidia/cuda:11.2.1-cudnn8-devel-ubuntu18.04 as darknet
LABEL mantainer="Jaled Moustafa <j.moustafa@sialitech.com>"
...
RUN cd /opt/opencv && mkdir release && cd release && \
    ...
    -DWITH_CUDA=OFF -DOPENCV_EXTRA_MODULES_PATH=../../opencv_contrib/modules \
    .. &&\
    make -j"$(nproc)"  && \
    make install && \
    ldconfig &&\
